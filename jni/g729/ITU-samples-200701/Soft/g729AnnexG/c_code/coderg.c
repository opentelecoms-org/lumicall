...
/*--------------------------------------------------------------------------------------*
 * Main program of the ITU-T G.729G   11.8/8 kbit/s encoder.
 *
 *    Usage : coderg speech_file  bitstream_file  DTX_flag [bit_rate or file_bit_rate]
 *--------------------------------------------------------------------------------------*/
...
int main(int argc, char *argv[] )
{
    ...
    printf("\n");
    printf("*************************************************************************\n");
    printf("****    ITU G.729 ANNEX G : 8.0, and 11.8 KBIT/S SPEECH CODER   ****\n");
    ...
    printf("\n");
    printf("                 Bit rates : 8.0, or 11.8 kb/s \n");
    printf("\n");
    ...
    *--------------------------------------------------------------------------*/
    if (( argc != 4 ) && (argc != 5) ){
        printf("Usage : coderg speech_file bitstream_file DTX_flag [bitrate or file_bit_rate]\n");
        ...
        printf("\n");
        printf("bitrate = 1 (8 kb/s)  or 2 (11.8 kb/s)  (default : 8 kb/s)\n");
        printf("Format for bitrate_file:\n");
        printf("  1 16bit-Word per frame , =1 bit rate 8 kb/s, or =2 bit rate 11.8 kb/s \n");
        ...
        if ( (f_rate = fopen(argv[4], "rb")) == NULL) {
            ...
            if( rate == 2) printf(" Selected Bitrate      :  11.8 kb/s\n");
                ...
                if( rate == 1) printf(" Selected Bitrate      :  8.0 kb/s\n");
                    ...
                    printf(" argv[4] = 2 for bit rate 11.8 kb/s\n");
                    exit(-1);
                }
            }
            flag_rate = 0;
    ...
    */
    ...
    while( fread(new_speech, sizeof(Word16), L_FRAME, f_speech) == L_FRAME) {
        ...
        Coder_ld8g(prm, frame, dtx_enable, rate);
        
        prm2bits_ld8g( prm, serial);
