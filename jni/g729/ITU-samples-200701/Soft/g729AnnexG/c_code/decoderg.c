 ...
 *
 *    Usage : decoderg bitstream_file  output_file
 *--------------------------------------------------------------------------------------*/
...
int main(int argc, char *argv[] )
{
    ...
    printf("\n");
    printf("*************************************************************************\n");
    printf("****    ITU G.729 ANNEX G : 8.0, and 11.8 KBIT/S SPEECH DECODER   ****\n");
    ...
    /* Passed arguments */
    if ( argc != 3 ) {
        printf("Usage : decoderg bitstream_file  output_file  \n");
        printf("\n");
    ...
    *-----------------------------------------------------------------*/
    while( fread(serial, sizeof(Word16), 2, f_serial) == 2) {
        ...
        if(serial_size != 0) {
            ...
            if( (serial_size != RATE_8000) && (serial_size != RATE_11800)) {
        ...
        if (parm[0] == 1) {
            ...
            if(serial_size < RATE_8000) serial_size = 0;
         }

        /* ---------- */
        /*  Decoding  */
        /* ---------- */
        Decod_ld8g(parm, voicing, synth_buf, Az_dec, &T0_first, &bwd_dominant,
            &m_pst, &Vad);
        ...
        for(i=0; i<L_FRAME; i+=L_SUBFR) {
            Postg(T0_first, &synth[i], ptr_Az, &pst_out[i], &sf_voic,
