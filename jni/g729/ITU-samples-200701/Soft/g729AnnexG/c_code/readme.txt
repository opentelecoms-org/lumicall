...
/*
   ...
   implementation of G.729 CS-ACELP at 8 kbit/s and 11.8 kb/s
   with DTX functionality
...
-----------
This is version 1.2
ITU-T G.729/Annex G was approved on 02/2000
...
version 1.1 line 176 :
---------------------------------------------------------
for(i=0; i<4; i++) L_exc_err[i]=0x00004000L;	/* Q14 */
---------------------------------------------------------
is replaced in v1.2 line 173 by :
---------------------------------------------------------
/* to tame the coder */
Init_exc_err();
---------------------------------------------------------
...
---------------------------------------
This software package is provided as part of ITU-T Rec. G.729 Annex G.

   ITU-T G.729 Annex  G fixed point ANSI C source code
...
Edit the makefiles coderg.mak and decoderg.mak to set the proper options
for your system.
...
version of the G.729 annex G  codec with higher bit rate
extension at 11.8 kbit/s and with dtx functionality

It also includes:
- the PC executable (coderg.exe and decoderg.exe):
- a batch file (test.bat) and testvectors to validate the correct
  execution of the G.729 annex G (ANSI-C software version 1.1). The
...
contains the value serial_size which is 80 for rate 8 kbit/s, 
118 for rate 11.8 kbit/s, 15( or 16) for SID frame, 0
