...
/* from dec_ld8k.c G.729 Version 3.3            */

/*-----------------------------------------------------------------*
 *   Functions Init_Decod_ld8g  and Decod_ld8g                     *
 *-----------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
        ...
        for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR) {
            ...
            *-------------------------------------------------------*/
            if(bfi != 0) {        /* Bad frame */
				...
				if (rate == G729E) {
					parm[2] = Random(&seed_fer);
					parm[3] = Random(&seed_fer);
                    parm[4] = Random(&seed_fer);
                }
            }
            ...
            *-------------------------------------------------*/
			...
			Dec_gaine(index, code, L_SUBFR, bfi, &gain_pitch, &gain_code,
				rate-(Word16)1, gain_pit_mem, gain_cod_mem, &c_muting,
