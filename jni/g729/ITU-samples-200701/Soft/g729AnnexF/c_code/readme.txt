...
-----------
This is version 1.2
ITU-T G.729/Annex F was approved on 02/2000
...
version 1.1 line 154 :
---------------------------------------------------------
for(i=0; i<4; i++) L_exc_err[i]=0x00004000L;	/* Q14 */
---------------------------------------------------------
is replaced in v1.2 line 151 by :
---------------------------------------------------------
/* to tame the coder */
Init_exc_err();
---------------------------------------------------------
...
---------------------------------------
This software package is provided as part of ITU-T Recommendation G.729 Annex F.
...
Edit the makefiles coderf.mak and decoderf.mak to set the proper options
for your system.
...
version of the  G.729 annex F codec with lower bit rate
extension at 6.4 kbit/s and with dtx functionality

It also includes:
- the PC executable (coderf.exe and decoderf.exe):
- a batch file (test.bat) and testvectors to validate the correct
  execution of  G.729 annex F (ANSI-C software version 1.1). The
  binary reference files are in PC format (for more details see
  readmetv.txt).

5.3 - USAGE
-----------
coderf  inputfile bitstreamfile rate_option
...
The following parameters are used for the encoder
	...
	rate_option = 0 : lower rate (6.4 kb/s)
				...
				= file_rate_name : a binary file of 16 bit 
                                  word containing either 0 or  1 to
...
rate 6.4 kbit/s, 15( or 16) for SID frame, 0
