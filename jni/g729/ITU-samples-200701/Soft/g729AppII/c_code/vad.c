...
/* C code for the modified VAD for Appendix II to G.729 */
/*
                       Alcatel, Texas Instruments
*/
#include <stdio.h>
#include "typedef.h"
#include "ld8a.h"
...
static Word16 flag, less_count;

#ifdef VAD_APPENDIX_II
static Word16  count_inert;
#else
static Word16  v_flag;
#endif
         ...
         Word16 *marker)
{
 /* scalar */
  ...
  if (sub(frm_count, INIT_FRAME) >= 0){
    if (sub(frm_count, INIT_FRAME) == 0){
#ifdef VAD_APPENDIX_II
      if (frm_count == INIT_FRAME && less_count >= INIT_FRAME) {
      	frm_count = 0;
      	less_count = 0;
      	}
#endif
    ...
    if((*marker == NOISE)&&(sub(count_inert,6) < 0))
	  {count_inert++;
	  *marker = VOICE;}
	#else
    v_flag = 0;
